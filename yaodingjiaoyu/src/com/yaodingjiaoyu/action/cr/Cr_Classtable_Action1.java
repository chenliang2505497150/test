...
public class Cr_Classtable_Action1 {
	...
	public String execute() {
		try {
			...
			stu_list = searchStudentService.findByCrRequest("", "", "no", "", "", "1", stuff, campus);

			//可以获得指定日期的周一和周日
			day =  dayService.getMondayAndSundayOfWeek(new Date());
			
			//获得课表
			List<ClassTable> list = classTableService.findClassTableByCr(day.get("one"), day.get("seven"), campus, null);
			...
			logger.error(this.getClass().getName()+"-->execute:添加合同失败,MESSAGE:"+e.getMessage());
			return "error";
		}
	}
}
