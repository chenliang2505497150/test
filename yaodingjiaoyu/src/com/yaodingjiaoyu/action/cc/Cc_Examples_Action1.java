...
public class Cc_Examples_Action1 {
	...
	public String execute(){
		try {
			...
			//未经处理，例如年级还是数字
			List<Map<String, Object>> list= searchExamplesService.findByCcRequest_MAP("", "", "no", "", "", "", "no", "no", "no", "no", campus, stuff, "no");
			//处理，将部分的内容进行翻译
			...
			logger.error(this.getClass().getName()+"-->execute:初始化例子失败。参数:"+",MESSAGE:"+e.getMessage());
			return "error";
		}
	}
}
