...
public class Sd_Examples_Action1 {
	...
	private List<Stuff> cc_list;
	
	
	public List<Stuff> getCc_list() {
		return cc_list;
	}

	public void setCc_list(List<Stuff> cc_list) {
	...
	private String getExamplesList() {
		...
		//初始化列表
		cc_list = loadPageListService.getSalerListByCampus(campus+"");
		...
		//未经处理，例如年级还是数字
		List<Map<String, Object>> list= searchExamplesService.findBySdRequest_MAP("", "", "no", "", "", "", "no", "no", "no", "no", campus+"","no","no","no");
		//处理，将部分的内容进行翻译
