...
public class CrSearchStudent_Ajax {
	...
	private void searchStudent() {
		...
		List<Map<String, Object>> list= searchStudentService.findByCrRequest_MAP(name, school, level, now_class, telephone, stu_status, isend, stuff, campus);
		//处理，将部分的内容进行翻译
		list = transLateService.transLateStudent(level_list, list);
		
		//初始化各个列表默认的信息
		studentList = (List<Map<String, Object>>) getResultObjectListService.getResultObjectList(start_look, PAGE_MAX, list);//显示指定页数据
	...
	public String execute(){
		...
		try {
			...
			logger.error(this.getClass().getName()+"-->execute:查找学生失败。参数start_look："+start_look+",name:"+name+
					",school:"+school+",level:"+
					level+",telephone:"+telephone+",isend:"+isend+",now_class:"+now_class+",stu_status:"+stu_status
