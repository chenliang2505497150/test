...
public class Cr_GetSimpleContent_Ajax {
	...
	private void  getContent() {
		...
		List<ClassContent> list = searchClassContentService.findCrContentByStudentId(id, stuff, campus);
		List<ClassContent> temp = (List<ClassContent>) getResultObjectListService.getResultObjectList(start_look, PAGE_MAX, list);//显示前15行数据
	...
	public String execute(){
		...
		try {
			...
			logger.error(this.getClass().getName()+"-->execute:确认上课失败。参数id："+id+",start_look:"+start_look+",MESSAGE:"+e.getMessage());
			return "error";
		}
		return null;
		}
