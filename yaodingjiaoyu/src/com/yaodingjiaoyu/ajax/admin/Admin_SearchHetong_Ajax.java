...
public class Admin_SearchHetong_Ajax {
	...
	public String execute(){
		...
		try {
			...
			//未经处理，例如年级还是数字
			List<Hetong> list= searchHetongService.findHetongByAdmin(hetong_num, name, school, level, hetong_type, campus, stuff, course_type, time1, time2);
			List<Hetong> tmp = (List<Hetong>) getResultObjectListService.getResultObjectList(start_look, PAGE_MAX, list);//显示前15行数据
			Contennt_list = transLateService.transLateHetongList(tmp);
			...
			//resultMap.put("total_money", item.get("allItem"));
			...
			logger.error(this.getClass().getName()+"-->execute:查询合同信息失败。参数start_look："+start_look+",time1:"+time1+
					",time2:"+time2+",name:"+name+",campus:"+campus+",school:"+school+",level:"+level+",hetong_type:"+
					hetong_type+",hetong_num:"+hetong_num+",course_type:"+course_type+",stuff:"+stuff+",MESSAGE:"
					+e.getMessage());
			return "error";
		}
		return null;
