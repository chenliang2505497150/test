...
public class Admin_SearchClassContent_Ajax {
	...
	public String execute() {
		...
		try {
			...
			//未经处理，例如年级还是数字
			List<ClassContent> list= searchClassContentService.findClassContentByAdmin(campus, time1, time2, stuff, name);
			
			List<Map<String, Object>> tmp = transLateService.transLateClassContent(list);
			Contennt_list = (List<Map<String, Object>>) getResultObjectListService.getResultObjectList(start_look, PAGE_MAX, tmp);//显示要求行数据
			...
			logger.error(this.getClass().getName()+"-->execute:查询跟踪记录失败。参数start_look："+start_look+",time1:"+time1+
					",time2:"+time2+",stuff:"+stuff+",campus:"+campus+",name:"+name+",MESSAGE:"+e.getMessage());
			return "error";
		}
		return null;
	}
