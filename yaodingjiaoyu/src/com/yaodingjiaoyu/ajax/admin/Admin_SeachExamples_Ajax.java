...
//处理例子库查询的筛选框 
public class Admin_SeachExamples_Ajax {
	...
	public String execute(){
		...
		try {
			...
			//未经处理，例如年级还是数字
			List<Map<String, Object>> list= searchExamplesService.findByAdminRequest_MAP(name, school, level, now_class, telephone, address, youxiao, zhuangtai, probability, channel, campus, cc_total);
			//处理，将部分的内容进行翻译
			list = transLateService.transLateExamples(level_list, probability_list, channel_list, campus_list, list);
			
			examples_list = (List<Map<String, Object>>) getResultObjectListService.getResultObjectList(start_look, PAGE_MAX, list);//显示要求行数据
			...
			logger.error(this.getClass().getName()+"-->execute:查询例子列表失败。参数name："+name+",school:"+school+
					",address:"+address+",telephone:"+telephone+",now_class:"+now_class+",level:"+
					level+",youxiao:"+youxiao+",zhuangtai:"+zhuangtai+",probability:"+probability
					+",channel:"+channel+",campus:"+campus+",cc_total:"+cc_total+",MESSAGE:"+e.getMessage());
			return "error";
		}
		return null;
	}
