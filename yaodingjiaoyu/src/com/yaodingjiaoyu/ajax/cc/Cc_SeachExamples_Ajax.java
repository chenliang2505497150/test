...
public class Cc_SeachExamples_Ajax {
	private String name;
	...
	public String execute() {
		...
		try {
			...
			//未经处理，例如年级还是数字
			List<Map<String, Object>> list= searchExamplesService.findByCcRequest_MAP(name, school, level, now_class, telephone, address, youxiao, zhuangtai, probability, channel, campus, stuff, cc_total);
			//处理，将部分的内容进行翻译
			...
			logger.error(this.getClass().getName()+"-->execute:查询例子失败。参数telephone："+telephone+",school:"+school+
					",name:"+name+",level:"+level+",now_class:"+now_class+",address:"+
					address+",youxiao:"+youxiao+",zhuangtai:"+zhuangtai+",probability:"+probability
					+",channel:"+channel+",cc_total:"+cc_total+",start_look:"+start_look+",MESSAGE:"+e.getMessage());
			return "error";
		}
		return null;
	}
