...
public class Cc_SaveExamples_Aajx {
	...
	public String execute(){
		...
		try {
			...
			stuff=  (int) ActionContext.getContext().getSession().get("ID");
			campus= (int) ActionContext.getContext().getSession().get("campus");
			...
			logger.error(this.getClass().getName()+"-->execute:初始化学生列表失败。参数phone1："+phone1+",school:"+school+
					",name:"+name+",level:"+level+",now_class:"+now_class+",address:"+
					address+",phone2:"+phone2+",zhuangtai:"+zhuangtai+",probability:"+probability
					+",channel:"+channel+",stuff:"+stuff+",campus:"+campus+",MESSAGE:"+e.getMessage());
			return "error";
