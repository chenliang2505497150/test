...
public class Sd_SaveHetong_Ajax {
	...
	public boolean checkInput() {
		...
		if ((course_type == 0) || (ht_property == 0) || (normal_hour == 0) || (subject == 0) || (now_class == 0)
				...
				|| ("".equals(phone1)) || ("".equals(parent_name)) || ("".equals(address)) || ("".equals(hetong_num))) {
			success = false;
		} else {
			success = true;
		}
	...
	public void createStuAndCTB(int campus) {
		// 新建学生、并获取编号
		student_id = saveStudentService.saveStudent(name, sex, birthday, school, campus, phone1, phone2, campus,
	...
	public void doRequest() {
		...
		if (checkInput()) {// 健壮性判断
			...
			if (student_id != 0) {
				...
				updateStuAndCTB();
				//不回滚添加合同
				AddHetong_MethodTwo(campus);
			} else {
				createStuAndCTB(campus);
				//可以回滚添加合同
				AddHetong_MethodOne(campus);
			}
	...
	//带回滚学生信息的插入
	public void AddHetong_MethodOne(int campus) {
		if (classhour_id != 0) {
			// 更新合同表
	...
	//不带回滚学生信息的插入
		public void AddHetong_MethodTwo(int campus) {
			if (classhour_id != 0) {
				// 更新合同表
				hetong_id = saveHetongService.saveHetong(1, student_id, hetong_num, level, subject, normal_hour,
						unit_price, ht_property, course_type, pos, cash, pos_num, receipt_num, stuff, campus,
	...
	public String execute() {
		...
		try {
			...
			logger.error(this.getClass().getName() + "-->execute:保存合同失败。参数name：" + name + ",sex:" + sex + ",birthday:"
					+ birthday + ",school:" + school + ",level:" + level + ",now_class:" + now_class + ",phone1:"
					+ phone1 + ",phone2:" + phone2 + ",parent_name:" + parent_name + ",address:" + address
					+ ",hetong_num:" + hetong_num + ",subject:" + subject + ",course_type:" + course_type
					+ ",normal_hour:" + normal_hour + ",ht_property:" + ht_property + ",course_type:" + course_type
					+ ",pos:" + pos + ",cash:" + cash + ",pos_num:" + pos_num + ",receipt_num:" + receipt_num
					+ ",remarks:" + remarks + ",stuff:" + stuff + ",SdId:"
