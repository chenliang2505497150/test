...
public class Sd_SearchCcContent_Ajax {
	private int start_look;// 本次浏览的起始页

	private String time1 = "";
	private String time2 = "";
	private String stuff;// 销售人员的编号
	...
	private void doSearchContent() {
		...
		// 未经处理，例如年级还是数字
		List<CcContent> list = searchCcContentService.findCcContentByAdmin(campus + "", time1, time2, stuff);

		List<Map<String, Object>> tmp = transLateService.transLateCcContent(list);
		ccContennt_list = (List<Map<String, Object>>) getResultObjectListService.getResultObjectList(start_look,
				PAGE_MAX, tmp);// 显示要求行数据
	...
	public String execute() {
		...
		try {
			...
			logger.error(this.getClass().getName() + "-->execute:保存合同失败。参数start_look：" + start_look + ",stuff:" + stuff
					+ ",time1:" + time1 + ",time2:" + time2 + ",SdId:"
