...
public class Sd_UnfreezeStuffList_Ajax {
	...
	private void doExecute() {
		...
		while (it.hasNext()) {
			...
			// 确认上课
			unfreezeStuff(Integer.parseInt(value));
	...
	public void unfreezeStuff(int id) {
		...
		if (null != stuff) {
			if (!accessManagerService.SdToStuff(power, campus, stuff)) {
				...
				if (!success) {
					resultMap.put("message", resultMap.get("message") + "编号:" + id + "解冻失败,");
				}
			}
		} else {
	...
	public String execute() {
		try {
			...
			logger.error(this.getClass().getName() + "-->execute:解冻员工账号。参数stuff_list_json：" + stuff_list_json + ",SdId:"
