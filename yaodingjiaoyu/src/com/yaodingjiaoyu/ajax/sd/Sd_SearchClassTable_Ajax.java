...
public class Sd_SearchClassTable_Ajax {
	...
	private void doSearchClassTable() {
		...
		List<ClassTable> list = classTableService.findClassTable(subject, tr_list_tmp, cr, student, time1, time2,
				status, class_time, campus, course_type, level);
		List<ClassTable> tmp = (List<ClassTable>) getResultObjectListService.getResultObjectList(start_look, PAGE_MAX,
				list);// 显示前15行数据
	...
	public String execute() {
		...
		try {
			...
			logger.error(this.getClass().getName() + "-->execute:查询课表失败。参数student：" + student + ",teacher:" + teacher
					+ ",time1:" + time1 + ",time2:" + time2 + ",level:" + level + ",subject:" + subject + ",class_time:"
					+ class_time + ",course_type:" + course_type + ",status:" + status + ",cr:" + cr + ",SdId:"
