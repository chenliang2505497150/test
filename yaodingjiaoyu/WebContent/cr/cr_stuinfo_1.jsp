	...
	function LoadData(result_obj){
		...
		if(result_obj.studentList != null){
			...
			for(var i=0;i<result_obj.studentList.length;i++){
				...
				var td5 = "<td class = 'center'>"+result_obj.studentList[i].now_class+"</td>";
				var td6 = "<td class = 'center'>"+result_obj.studentList[i].keshi+"</td>";
				var td7 = "<td class = 'center'>"+result_obj.studentList[i].phone1+"</td>";
				var td8 = "<td class = 'center'>"+result_obj.studentList[i].phone2+"</td>";
				var td9 = "<td class = 'center'>"+result_obj.studentList[i].total+"</td>";
				var td10_start = "<td class = 'center'>";
				var td10_end = "</td>";
				var div_start = "<div>";
				var div_end = "</div>";
				var select_start = "<select class='selectbox' name='operate' id ='"+result_obj.studentList[i].P_ID+"'  style='width:100px' >";
				var option = "<option value='no'>－－－－</option><option value='cr_add_content'>添加回访</option><option value='student_detail'>详情</option><option value='cr_add_hetong'>续费</option>";
				var select_end = "</select>";
				var td10 = td10_start + div_start + select_start + option + select_end + div_end + td10_end;
				var tr_end = "</tr>";
									    ...
									    <div class="collapse navbar-collapse" id="bs-example-navbar-collapse-1">
									      <form class="navbar-form navbar-left" role="search" name = "seachstu">
									        <div class="form-group">
									            ...
									            学员状态：<s:select list="#{'0':'已结课','1':'未结课'}" headerKey="no" headerValue="－－－－"  class="selectbox" name = "stu_status" id = "stu_status" style="width:100px"/>
									            <p>
									            排课状态：<s:select list="#{'0':'已排完','1':'未排完'}" headerKey="no" headerValue="－－－－"  class="selectbox" name = "isend"  style="width:100px"/>
	                            ...
	                            <div class="table-responsive">
	                                ...
	                                <table class="table table-striped table-bordered table-hover" id="dataTables-example">
	                                    ...
	                                    <s:iterator value="studentList" id = "student" status = "sta">
	                                    	<s:set name="id" value="#student.P_ID"/>  <!-- 定义存储ID的变量 -->
	                                    	<s:set name="num" value="item.firstItem"/>
	                                    	<tr class="odd gradeX">
	                                            <td><s:property value="#num+#sta.count-1"/></td>
	                                            <td><s:property value="#student.name"/></td>
	                                            <td><s:property value="#student.school"/></td>
	                                            <td class="center"><s:property value="#student.level"/></td>
	                                            <td class="center"><s:property value="#student.now_class"/></td>
	                                            <td class="center"><s:property value="#student.keshi"/></td>
	                                            <td class="center"><s:property value="#student.phone1"/></td>
	                                            <td class="center"><s:property value="#student.phone2"/></td>
	                                            <td class="center"><s:property value="#student.total"/></td>
	                                            <td class="center">
		                                            <div>
		                                            		<select class="selectbox" name="operate" id ="${id}"  style="width:100px" >  
					                                        		<option value="no">－－－－</option>
					                                                <option value="cr_add_content">添加回访</option>
					                                                <option value="student_detail">详情</option>
					                                                <option value="cr_add_hetong">续费</option>
					                                        </select>
		                                            </div>
	                                    		</td>
	                                        </tr>
