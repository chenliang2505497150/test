...
Bind();
	...
	$("a[name=submit_choose]").click(function(){
		...
		if(student_list.length == 0){
			...
			if($('select[name=cr] option:selected').val() == "no"){
				alert("未选择班主任");
			}else{
				...
				$.ajax({
	                type: "post",
	                url: "sd_distribution_student",
	                async: false,
	                data: { 
	                	student_list_json: JSON.stringify(json),
	...
	function LoadData(result_obj){
		...
		if(result_obj.studentList != null){
			...
			for(var i=0;i<result_obj.studentList.length;i++){
				...
				var td0 = "<td><div class='checkbox3 checkbox-success checkbox-inline checkbox-check checkbox-round  checkbox-light'><input type='checkbox' name = 'check_content' value ='"+result_obj.studentList[i].P_ID+"' id='checkbox-fa-light-"+result_obj.studentList[i].P_ID+"'><label for='checkbox-fa-light-"+result_obj.studentList[i].P_ID+"'></label></div></td>";
				...
				var option = "<option value='no'>－－－－</option><option value='sd_add_operate'>添加合同</option><option value='sd_change_student'>修改</option><option value='student_detail'>详情</option>";
				var select_end = "</select>";
				var td8 = td8_start + div_start + select_start + option + select_end + div_end + td8_end;
				var tr_end = "</tr>";
	            
				var html = tr_start + td0 + td1 + td2 + td3 + td4 + td5 + td6 + td7 +td8+ tr_end;
					...
					<div class="dialog__content">
						...
						<div>
							<table class="table table-striped table-bordered table-hover" id = "savedata" stuff="" pram="">
								<tr class="odd gradeX">
									<td>班主任:</td>
									<td><s:select list="stuff_list" listKey="PId" listValue="name" headerKey="no" headerValue="－－－－" class="selectbox" name = "cr" style="width:200px" theme="simple"/></td>
								</tr>
								<tr class="odd gradeX">
									<td colspan="2" align="center">
										<a href="javascript:void(0);" class="btn btn-warning action" data-dialog-close>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<span id="close_devide">关闭</span>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</a>
									    ...
									    <div class="collapse navbar-collapse" id="bs-example-navbar-collapse-1">
									      <form class="navbar-form navbar-left" role="search" name = "seachstu">
									        <div class="form-group">
								            	...
								            	班主任:<s:select list="stuff_list" listKey="PId" listValue="name" headerKey="no" headerValue="－－－－" class="selectbox" name = "stuff" style="width:100px" theme="simple"/>
	                                            ...
	                                            <td class="center">
		                                            <div>
		                                            		<select class="selectbox" name="operate" id ="${id}"  style="width:100px" >  
					                                                ...
					                                                <option value="sd_add_operate">添加合同</option>
					                                                <option value="sd_change_student">修改</option>
