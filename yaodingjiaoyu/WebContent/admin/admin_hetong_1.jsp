...
Bind();
		...
		$("select[name=operate]").each(function(){
			$(this).change(function(){
				...
				if($(this).val()!="no"){
					...
					//判断是不是删除事件,如果是删除事件就发送Ajax
					if($(this).val() == "delete_hetong"){
						var message = "您确定要执行<删除>操作吗?";
	...
	function DeleteHetong(hetong_id){
	...
	$("select[name=campus]").change(function(){
		if($(this).children('option:selected').val()=="no"){
			//清空cc列表数据
			$("select[name=stuff]").empty();//先清空cc列表
			var option = "<option value='no'>－－－－</option>";
			$("select[name=stuff]").append(option);
		}
	...
	function LoadData(result_obj){
		...
		if(result_obj.Contennt_list != null){
			...
			for(var i=0;i<result_obj.Contennt_list.length;i++){
				...
				var td11 = "<td class = 'center'>"+result_obj.Contennt_list[i].stuff+"</td>";
				var td12 = "<td class = 'center'>"+result_obj.Contennt_list[i].campus+"</td>";
				...
				var option = "<option value='no'>－－－－</option><option value='hetong_detail'>详情</option><option value='change_hetong'>修改合同</option><option value='delete_hetong'>删除合同</option>";
				var select_end = "</select>";
				var tdmore = tdmore_start + div_start + select_start + option + select_end + div_end + tdmore_end;
				var tr_end = "</tr>";
	            
				var html = tr_start + td1 + td2 + td3 + td4 + td5 + td6 +td7+ td8 + td9 + td10 + td11 +td12+tdmore+ tr_end;
									    ...
									    <div class="collapse navbar-collapse" id="bs-example-navbar-collapse-1">
									      <form class="navbar-form navbar-left" role="search" name = "search_content">
									        <div class="form-group">
								            	...
								            	签约人:<s:select list="#{'no':'－－－－'}"   class="selectbox" name = "stuff" style="width:100px" theme="simple"/>
	                                            ...
	                                            <td class="center">
		                                            <div>
		                                            		<select class="selectbox" name="operate" id ="${id}"  style="width:100px" >  
					                                        		...
					                                        		<option value="hetong_detail">详情</option>
					                                                <option value="change_hetong">修改合同</option>
					                                                <option value="delete_hetong">删除合同</option>
					                                        </select>
		                                            </div>
	                                    		</td>
	                                        </tr>
