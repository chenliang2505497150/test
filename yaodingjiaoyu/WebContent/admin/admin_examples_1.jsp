...
Bind();
		...
		$("select[name=operate]").each(function(){
			$(this).change(function(){
				...
				if($(this).val()!="no"){
					...
					if($(this).val() == "admin_examples_delete"){
						DeleteExamples(ex_Id);
						//刷新数据
						getPageAjax();
					}else{
						//跳转
	...
	function DeleteExamples(examples_id){
		
		// 发送Ajax请求
        $.ajax({
                type: "post",
                url: "admin_delete_examples",
                async: false,
                data: { 
                	id: examples_id,
                },
	...
	function LoadData(result_obj){
		...
		if(result_obj.examplesList != null){
			...
			for(var i=0;i<result_obj.examplesList.length;i++){
				...
				var option = "<option value='no'>－－－－</option><option value='examples_detail'>详情</option><option value='admin_examples_delete'>删除</option><option value='admin_examples_change'>修改</option>";
	...
	$("button[name=reset]").click(function(){
		...
		$("select[name=campus] option:first").prop("selected","selected");
									    ...
									    <div class="collapse navbar-collapse" id="bs-example-navbar-collapse-1">
									      <form class="navbar-form navbar-left" role="search" name = "searchexam">
									        <div class="form-group">
									        	...
									        	住址：<input type="text" class="form-control" placeholder="Search" name = "address">
								            	<p>       
	                                            ...
	                                            <td class="center">
		                                            <div>
		                                            		<select class="selectbox" name="operate" id ="${id}"  style="width:100px" >  
					                                                ...
					                                                <option value="admin_examples_delete">删除</option>
					                                                <option value="admin_examples_change">修改</option>
					                                        </select>
		                                            </div>
	                                    		</td>
	                                        </tr>
